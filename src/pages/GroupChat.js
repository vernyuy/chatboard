...
import { ChatRoom, GroupMember, Message, User } from "../models";
...
import GroupChatScreen from "../components/GroupdChatScreen";
...
function GroupChat(){
  ...
  const {user} = useSelector(state=>state.user)
    ...
    useEffect(()=>{
        // async ()=>{
             DataStore.query(ChatRoom).then(data=>{
                setGroups(data)
                // console.log(data)
            })

            DataStore.query(User,(user)=>user.cognitoId.eq(
              DataStore.query(GroupMember)
            ))
            const subscription = DataStore.observeQuery(GroupMember).subscribe(snapshot => {
          ...
          <div className=" w-full h-5/6 my-2">
                <div className="sm:grid sm:grid-cols-3 md:grid-cols-4 h-full my-2 h- gap-3 bg-white rounded-s-xl">
                    ...
                    <div className="w- bg-gray-100  rounded-xl m-3 overflow-scroll sm:col-span-2 md:col-span-3">
                        {
                        userId?
                            <GroupChatScreen senderId={userId}/>:
